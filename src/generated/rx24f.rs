    ...
    Serial: serial::Write<u8> + serial::Read<u8>,
{
    ...
    fn set_rx24f_id(&mut self, id: u8, params: u8) -> Result<Option<StatusPacket<1>>, Error<Serial>> {
    ...
    fn set_rx24f_led(&mut self, id: u8, params: u8) -> Result<Option<StatusPacket<1>>, Error<Serial>> {
        let mut content: Vec<u8, 3> = Vec::new();
        content.push(25).map_err(|_| Error::TooSmall)?;
        if PROTOCOL_VERSION == 2 {
            content.push(0).map_err(|_| Error::TooSmall)?;
    ...
    fn set_rx24f_lock(&mut self, id: u8, params: u8) -> Result<Option<StatusPacket<1>>, Error<Serial>> {
        let mut content: Vec<u8, 3> = Vec::new();
        content.push(47).map_err(|_| Error::TooSmall)?;
        if PROTOCOL_VERSION == 2 {
            content.push(0).map_err(|_| Error::TooSmall)?;
